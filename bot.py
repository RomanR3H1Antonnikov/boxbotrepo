    ...
    while True:
        try:
            ...
            if not orders_to_check:
                logger.debug("Нет shipped заказов для проверки")
            ...
            for detached_order in orders_to_check:
                with Session(engine) as sess:
                    order = sess.get(Order, detached_order.id)
                    if order is None:
                        continue
                    ...
                    if order.status == OrderStatus.SHIPPED.value and re.match(
                        ...
                        if uuid_to_poll:
                            ...
                            asyncio.create_task(poll_cdek_order_status(uuid_to_poll, order.id))
                        continue
                    ...
                    # Самое важное — entity!
                    # ───────────────────────────────────────────────
                    entity = info  # ← потому что get_cdek_order_info уже возвращает entity
                    if not entity:
                        logger.warning(f"Нет данных (entity) в ответе для заказа #{order.id} (uuid {uuid})")
                        continue
                    ...
                    logger.info(
                        ...
                        f"cdek_number={cdek_number} | status={current_status_desc}"
                    )

                    # Сохраняем трек-номер, если он появился и ещё не сохранён
                    if cdek_number and len(str(cdek_number)) >= 8 and (not order.track or order.track.startswith("BOX")):
                        ...
                        await notify_admin(
                            f"Трек-номер пришёл!\n"
                            f"Заказ #{order.id} → <code>{cdek_number}</code>\n"
                        )

                    # Уведомления об изменении статуса (только важные)
                    important_statuses = [
                    ...
                    if (current_status_desc in important_statuses and
                        ...
                        await bot.send_message(
                            ...
                            f"Трек: <code>{order.track or 'ожидается...'}</code>\n"
                            f"<a href='https://www.cdek.ru/ru/tracking?order_id={order.track or ''}'>Отследить</a>",
                            parse_mode="HTML",
                            disable_web_page_preview=True
                        )
            ...
            logger.exception(f"Критическая ошибка в check_all_shipped_orders: {e}")
